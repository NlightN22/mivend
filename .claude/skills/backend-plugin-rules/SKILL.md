...
- **A `customField` written only by backend code (a sync worker, an ERP import handler, a one-time provisioning step) must be declared `readonly: true` — never leave it as a plain writable field "because nothing edits it yet."** Without `readonly`, Vendure's native Dashboard (and any other GraphQL client) renders a perfectly normal editable text box for it on the entity's detail page — nothing in the schema signals "this is system-owned," so a human can and eventually will hand-edit it, silently breaking whatever correlation/sync mechanism owns that field (a changed `erpId` breaks the anchor an entire creation flow depends on; a changed `sourceAdministratorId` breaks a branch replica's link to its Central source; a changed `departmentId`/`branchId` desyncs from the next ERP import, which will just overwrite it back anyway, hiding the tamper). Real incident: `access-control.plugin.ts`'s `Administrator.customFields` — `erpId`, `departmentId`, `branchId`, `position`, `sourceAdministratorId` were all plain writable fields; the native Dashboard's Administrator detail page showed all five as editable inputs, `erpId` included, defeating the entire point of anchoring Administrator creation on it (issue #119, Decision 5). **`readonly: true` only strips the field from the generated `Create<Entity>CustomFieldsInput`/`Update<Entity>CustomFieldsInput` GraphQL input types** (verified against `@vendure/core`'s `addGraphQLCustomFields`/`addActiveAdministratorCustomFields`) — it does **not** block a direct TypeScript call to `<Entity>Service.create()`/`.update()` from your own plugin code, since that bypasses the GraphQL input type entirely. So the fix is free: mark it `readonly`, and every legitimate backend writer keeps working unchanged — only a human clicking around the Dashboard/Admin UI loses the (wrong) ability to edit it. When adding any new `customField`, ask "does a human ever legitimately type a value into this field itself, or does something else derive/own it?" — if the latter, `readonly: true` from the start, not as an afterthought once someone notices the exposed control.
...
- **Renaming/changing the bootstrap superadmin's identifier away from `SUPERADMIN_USERNAME` (without also updating that env var) makes Vendure silently recreate — or, worse, un-delete — a second superadmin on the next server restart.** `AdministratorService.ensureSuperAdminExists()` (`@vendure/core`) runs unconditionally on every app bootstrap: it looks up a `User` by `identifier === authOptions.superadminCredentials.identifier` (from `SUPERADMIN_USERNAME`/`SUPERADMIN_PASSWORD`, `apps/server/src/vendure-config.ts:71-73`) with **no `deletedAt` filter on that lookup** — if none is found (because the original account was renamed away from that identifier), it creates a brand new one with the env's default password; if one _is_ found but was soft-deleted, it silently sets `deletedAt = null` on both the `Administrator` and `User` rows, resurrecting it with its old password. Real incident (mivend#85's audit session, 2026-09-13): a superadmin renamed to `admin` on 2026-09-05 got a fresh, independent `superadmin`/`superadmin` account auto-created a week later on some later restart, because `.env.central.staging-integration`'s `SUPERADMIN_USERNAME` didn't get updated to `admin` until after that restart already happened — two real, both-working Administrator rows with the `SuperAdmin` role existed simultaneously, each with its own separate browser session (session cookies are scoped per frontend origin — `:8015` manager portal vs `:8016` native Dashboard — so each surfaced a _different_ logged-in identity with no visible contradiction until compared side by side). **Rule: the moment you rename/change the password of whichever Administrator is meant to be "the" superadmin for a contour, update that contour's `SUPERADMIN_USERNAME`/`SUPERADMIN_PASSWORD` in its `.env.*` file (and restart) to match exactly** — this env var isn't just a first-boot seed value, it's the identifier Vendure re-checks and self-heals against on _every_ restart, indefinitely. Cannot be fixed in mivend code (`@vendure/core`, "never modify Vendure core") — this is Vendure's own by-design lockout-prevention mechanism, not a bug to patch, only a footgun to know about. `apps/server/src/dashboard/default-superadmin-account/index.ts`'s alert (and the manager portal's matching `authStore.isDefaultSuperadminAccount` warning) only catches "I am currently logged in as the literal default `superadmin`/`superadmin`" — neither catches "a second, resurrected superadmin silently exists alongside the one I actually use."

- **Renaming an `@Entity()` class (or otherwise changing its implicit table name) under `synchronize: true` does NOT rename the table — it silently creates a brand-new, empty table under the new name and orphans the old one with all its data still in it, invisible to the app from that point on.** This project has no TypeORM migration files anywhere (`synchronize: true` for every non-production `NODE_ENV`, `apps/server/src/vendure-config.ts`) — a class rename is the _only_ way an entity's table changes shape here, and `synchronize` only ever creates/alters tables for currently-known entities; it never drops or renames a table that no longer has a matching entity. Real incident (issue #119/#104 follow-up, 2026-09-20): `PendingErpUser` → `ErpUser` renamed the entity class with no explicit `@Entity('pending_erp_user')` pin and no data-migration step; on staging-integration this **orphaned 1218 real, already-synced ERP candidate rows** in the now-invisible `pending_erp_user` table — the new `erp_user` table synchronized in empty, so every "Pending ERP users" list (Dashboard and manager portal both) correctly, faithfully reported zero rows, which looked exactly like "the data was deleted" to everyone watching, including the user testing live. Nothing was actually deleted; both tables coexisted in Postgres the whole time (confirmable via `\dt` in the running container) until someone manually re-`INSERT ... SELECT`-migrated the old table's rows into the new one's schema. **Rule: before renaming any `@Entity()` class, decide explicitly — either (a) pin the old table name via `@Entity('old_table_name')` so `synchronize` keeps writing to the same table (no data movement needed, only worth it if the column shape didn't also change), or (b) if the table name is meant to change for real, write and run a real migration (or at minimum a one-off `INSERT INTO new_table SELECT ... FROM old_table` before merging) that copies every row across, then drop the old table only after confirming the copy — never let `synchronize` "handle" a rename by itself.** This applies with extra weight to any table `erp-integration`'s inbox-driven handlers or a Kafka consumer write to on a contour connected to a real external broker (`staging-integration` per `docs/environments.md`) — an inbox message already marked `processed` will never be redelivered, so a rename-induced silent orphan on that contour is not just invisible until discovered, it's **unrecoverable via reprocessing** (the only recovery path left is exactly this kind of manual, after-the-fact data-copy triage). A quick way to sanity-check after any entity rename that has landed on a contour with real data: `docker exec <postgres-container> psql -U postgres -d <db_name> -c "\dt"` and look for _both_ the old and new table names still present — if the old one still exists and has rows, migrate them before calling the rename done.
