...
describe('OfflineTermsBootstrapService', () => {
    ...
    let service: OfflineTermsBootstrapService;
    ...
    beforeEach(() => {
        ...
        service = new OfflineTermsBootstrapService(
    ...
    it('creates the payment method when it does not exist yet', async () => {
        ...
        expect(input.code).toBe(OFFLINE_TERMS_METHOD_CODE);
        expect(input.handler.code).toBe(OFFLINE_TERMS_METHOD_CODE);
    });

    it('is idempotent: does not create a duplicate when the payment method already exists', async () => {
        repo.findOne.mockResolvedValue({ id: 1, code: OFFLINE_TERMS_METHOD_CODE });
        await service.onApplicationBootstrap();
