...
//
// Gated on ProcessContext.isWorker (issue #80) — both main.ts and worker.ts load this plugin
// unconditionally, and this is a recurring background sweep, not request-serving code, so it
// belongs on the worker process only, matching KafkaConsumerBootstrapService's own
// ProcessContext.isWorker convention and docs/architecture.md's worker-layer design.
@Injectable()
