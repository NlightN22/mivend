...
import { PICKUP_SHIPPING_METHOD_CODE } from '../../constants';
...
describe('PickupShippingBootstrapService', () => {
    ...
    let service: PickupShippingBootstrapService;
    ...
    beforeEach(() => {
        ...
        service = new PickupShippingBootstrapService(
    ...
    it('creates the shipping method when it does not exist yet', async () => {
        ...
        expect(input.code).toBe(PICKUP_SHIPPING_METHOD_CODE);
    ...
    it('is idempotent: does not create a duplicate when the shipping method already exists', async () => {
        repo.findOne.mockResolvedValue({ id: 1, code: PICKUP_SHIPPING_METHOD_CODE });
        await service.onApplicationBootstrap();
